...
export default function ResponsibilityRequestForm() {
    ...
    const fetchProducts = React.useCallback(async (term: string) => {
        setIsLoading(true);
        try {
            const productsFromDb = await getProducts({ searchTerm: term, materialType: 'permanente' });
    ...
    const handleAddItem = () => {
        ...
        if (!item) {
            toast({ title: "Item não encontrado", description: `O item buscado não existe no estoque de permanente.`, variant: "destructive" });
            return;
        }
        ...
        setRequestedItems((prev) => {
            ...
            return [...prev, { id: item.id, name: item.name, quantity, unit: item.unit, patrimony: item.patrimony }];
        });
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {requestedItems.map(reqItem => {
                                            const product = searchResults.find(p => p.id === reqItem.id);
                                            return (
